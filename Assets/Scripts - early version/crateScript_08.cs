...
public class crateScript_08 : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D coll){
		...
		if (coll.gameObject.tag == "Player") {
			...
			case "crates_0":
				GameObject.Find("Main Camera").GetComponent<levelCreator_08>().gameSpeed -=1.0f; // CHANGE V4 BEGINNING 8
				break;
			case "crates_1":
	...
	void placeCrate(){
		...
		GameObject tmpTile = GameObject.Find ("Main Camera").GetComponent<levelCreator_08> ().tilePos;
